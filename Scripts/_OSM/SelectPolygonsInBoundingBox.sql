...
--limiting to admin level 6 polygons 
...
SELECT
    -- Return the clipped geometry of the Admin Level 6 polygon
    AsText(
        ST_Intersection(
  ...
  WHERE
    p.polygonX IS NOT NULL
    AND :zoomBand <= p.displayMin 
    -- Spatial Index Filter
    AND p.ROWID IN (
        SELECT ROWID FROM SpatialIndex 
        WHERE f_table_name = 'spt_polygons' AND f_geometry_column = 'polygonX'
